...
def kalman_filter(z_meas, x_esti, P, R):
    # 1. Prediction
    x_pred = A @ x_esti
    P_pred = A @ P @ A.T + Q
...
xpos_100_esti_save = np.zeros(n_samples)
ypos_100_esti_save = np.zeros(n_samples)


x_esti, P = None, None
x_100_esti, P_100 = None, None
...
for i in range(n_samples):
    ...
    if i == 0:
        x_esti, P = x_0, P_0
        x_100_esti, P_100 = x_0, P_0
    else:
        x_esti, P = kalman_filter(z_meas, x_esti, P, R)
        x_100_esti, P_100 = kalman_filter(z_meas, x_100_esti, P, R_100)
    
    xpos_meas_save[i] = z_meas[0]
    ypos_meas_save[i] = z_meas[1]
    xpos_esti_save[i] = x_esti[0]
    ypos_esti_save[i] = x_esti[2]
    xpos_100_esti_save[i] = x_100_esti[0]
    ypos_100_esti_save[i] = x_100_esti[2]



fig = plt.figure(figsize=(8, 8))
plt.gca().invert_yaxis()
plt.scatter(xpos_100_esti_save, ypos_100_esti_save, s=300, c="r", marker='*', label='1/100 R')
plt.scatter(xpos_esti_save, ypos_esti_save, s=120, c="b", marker='o', label='Q')
...
plt.xlim((-10, 350))
plt.ylim((250, -10))
plt.savefig('png/object_tracking_R_value.png')